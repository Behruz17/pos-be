...
- `Authorization: Bearer <токен>`
**Параметры запроса (опционально):**
- `store_id` - ID магазина для фильтрации (админы могут указать любой магазин). Для обычных пользователей возвращаются только должники их магазина.
**Ответ:**
```json
[
  {
...
```json
{
  "customer_name": "Иван Петров",
  "phone": "+79991234567",
  "store_id": 1, // обязательный для админа; для обычного пользователя будет автоматически установлен ID его магазина
  "sale_id": 123,
  "amount": 5000.00
}
```
...
**Примечания по доступу и `store_id`:**
- Все `retail_debtors` теперь связаны с конкретным магазином через поле `store_id`.
- Обычные пользователи видят и могут изменять только должников своего магазина (определяется по `req.userStoreId`).
- Администратор может указывать `store_id` при создании или указывать `store_id` в параметрах GET для фильтрации.
...
## 13. Управление доставщиками

### GET /api/warehouses/:warehouseId/delivery-drivers
**Назначение:** Получение списка доставщиков для конкретного склада  
**Заголовки:**
- `Authorization: Bearer <токен>`
**Параметры:**
- `warehouseId` - ID склада
  ...
  "drivers": [
    {
      ...
      "status": 1,
      "warehouse_id": 11,
      "created_at": "2026-03-19T11:07:37.000Z",
      "updated_at": "2026-03-19T11:07:37.000Z"
    }
  ]
}
```
**Примечания:**
- Возвращаются только активные доставщики (status = 1)
- Доставщики фильтруются по указанному складу

### POST /api/warehouses/:warehouseId/delivery-drivers
**Назначение:** Создание нового доставщика для указанного склада  
**Заголовки:**
- `Authorization: Bearer <токен>`
**Параметры:**
- `warehouseId` - ID склада
...
{
  "name": "Имя доставщика",
  "phone": "123456789" (необязательно),
  "balance": 0.00 (необязательно, по умолчанию 0)
}
```
**Ответ:**
```json
...
```
**Ошибки:**
- `400 Bad Request`: Если не указано поле `name`
- `404 Not Found`: Если склад с указанным ID не найден

### PUT /api/delivery-drivers/:id
**Назначение:** Обновление информации о доставщике  
**Заголовки:**
- `Authorization: Bearer <токен>`
**Параметры:**
- `id` - ID доставщика
...
```json
{
  "name": "Новое имя доставщика" (необязательно),
  "phone": "987654321" (необязательно),
  "balance": 100.00 (необязательно),
  "status": 1 (необязательно)
}
```
**Ответ:**
```json
...
```
**Ошибки:**
- `400 Bad Request`: Если нет полей для обновления
- `404 Not Found`: Если доставщик не найден

### DELETE /api/delivery-drivers/:id
**Назначение:** Удаление доставщика (мягкое удаление)  
**Заголовки:**
- `Authorization: Bearer <токен>`
**Параметры:**
- `id` - ID доставщика
...
**Назначение:** Обновление стоимости доставки, валюты и курса для существующего прихода  
**Заголовки:**
- `Authorization: Bearer <токен>`
**Параметры:**
...
```
**Ошибки:**
- `400 Bad Request`: Если стоимость доставки <= 0, курс <= 0 или не указан ID водителя
- `404 Not Found`: Если приход не найден или у него нет водителя
**Примечания:**
- Если операция доставки уже существует - обновляется сумма, валюта и курс
- Если операции нет - создается новая с указанными параметрами
- `converted_sum` рассчитывается как `сумма_прихода / rate`
- Поле `rate` опционально, если не указано - `converted_sum` будет NULL
...
**Назначение:** Получение сводной статистики по всем реселлерам  
**Заголовки:**
- `Authorization: Bearer <токен>`
**Параметры запроса (опционально):**
- `store_id` - ID магазина для фильтрации (админы могут указать любой магазин). Для обычных пользователей возвращается статистика только их магазина.
**Ответ:**
```json
{
  "summary": {
    "total_receipts": 12000.00,
    "total_sales": 15000.00,
    "my_payments": 8000.00,
    "reseller_payments": 6000.00,
    "total_debt_balance": 3000.00,
    "active_resellers": 5
  },
  "breakdown": {
    "receipts_from_operations": 12000.00,
    "sales_from_operations": 15000.00,
    "payments_made_to_resellers": 8000.00,
    "payments_received_from_resellers": 6000.00,
    "outstanding_debt_balance": 3000.00
  }
}
```
**Поля ответа:**
- `total_receipts` - Общая сумма прихода (на сколько вы взяли у реселлеров, тип операции RECEIPT)
- `total_sales` - Общая сумма продажи (на сколько вы продали реселлерам, тип операции SALE)
- `my_payments` - Сумма оплат, которые вы заплатили реселлерам (PAYMENT_TO_RESELLER)
- `reseller_payments` - Сумма оплат, которые реселлеры заплатили вам (PAYMENT_FROM_RESELLER)
- `total_debt_balance` - Сумма всех текущих балансов реселлеров
- `active_resellers` - Количество активных реселлеров
**Примечания:**
- Администраторы могут указать `store_id` для получения статистики по конкретному магазину
- Обычные пользователи видят статистику только своего магазина
- Все расчеты фильтруются по указанному магазину